...
@EXPORT_OK = qw(md4 md4_hex md4_base64 password_to_vector);
...
# This implementation adds interfaces to overwrite the MD4 IV
...
{
    my ($data,@A) = @_;
    my @input = grep defined && length>0, split /(.{64})/s, join '', $data;
    push @input, '' if !@input || length($input[$#input]) >= 56;
    @A = @A ? @A : (0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476); # initial regs
...
sub password_to_vector($){
    my($pw)=@_;
    # little endian, will be swapped back by unpack()
    my $vector = "\x01\x23\x45\x67\x89\xab\xcd\xef\xfe\xdc\xba\x98\x76\x54\x32\x10";
    my $pw_bytes="";
    my $last_byte_number = length($pw) % 4;
    my $count;

    for($count = 0; $count < length($pw)/4; $count++){
        $pw_bytes.=substr($pw,$count*4,1);
        $pw_bytes.=substr($pw,$count*4 + 1 ,1);
        $pw_bytes.=substr($pw,$count*4 + 2,1);
        $pw_bytes.=substr($pw,$count*4 + 3,1);

    }

    for($count = $last_byte_number; $count >0 ; $count--){
        $pw_bytes.= substr($pw,length($pw) - $count, 1);
    }

    my $remainder = substr($vector,length($pw_bytes),16 - length($pw_bytes));

    $pw_bytes = $pw_bytes.$remainder;

    return unpack("L1L1L1L1",$pw_bytes);
}
